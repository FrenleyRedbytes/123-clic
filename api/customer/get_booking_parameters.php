...
if(@mysqli_num_rows($specialistQuery) > 0){
	...
	//print_r($finalData['serviceDetails']);die;
	...
	//print_r(@mysqli_fetch_array($checkWorkersQuery));die;
	...
	// appointment calendar starts here
			...
			// 		// before noon time slot starts here
			 		...
			 		for($i=$start_time*60+$start_time1[1];$i<=$end_time*60+$end_time1[1];$i+=$service_time){
						 ...
						 $a=mysqli_query($con,"select * from worker where spe_id='$specilaist_id' and cat_id=$spe_cat_id and subcat_id=$spe_sub_cat_id"); 
						 ...
						 $sum=0;
			 			 ...
			 			 while($data=mysqli_fetch_array($a)){
			 			 	$worker=$data['work_id']; 
			 				// get worker details starts here
			 			 	 // $workerArray['worker_id'] = $data['work_id'];
			 			 	 // $workerArray['worker_first_name'] = $data['first_name'];
			 			 	 // $workerArray['worker_last_name'] = $data['last_name'];
			 			 	 // $workerArray['worker_days_of_work'] = $data['day'];
			// 				//print_r($workerArray);die;
			// 				// worker details ends here
			...
			// 				//print_r($worker_id_with_service);die;
			 				...
			 				if($num_rows1 > 0){
			 			 		$checkWorkerServiceFlag = 1;
			 			 	}else{
			 			 		$checkWorkerServiceFlag = 0;
			 			 	}

			// 			 	//echo $checkWorkerServiceFlag;die;
			 			 	...
			 			 	if(!empty($data1['day_leave_start']) || !empty($data1['day_leave_end'])){
			 					...
			 					$num_rows11 = mysqli_num_rows($worker_holiday1);
			 			 		...
			 			 		if($num_rows11 > 0){
			 			 			$checkWorkerHolidayFlag = 1; 
			 			 		}else{
			 			 			$checkWorkerHolidayFlag = 0; 
			 			 		}	
			 			 	 ...
			 			 	 if(($checkWorkerHolidayFlag ==1) && ($checkWorkerServiceFlag ==1)){
			 	 	 			 	$workerData = array();
			 		 	 	 }else{
			...
			// 		// after noon time slot starts here
			 		...
			 		for($i=$start_time*60+$start_time1[1];$i<=$end_time*60+$end_time1[1];$i+=$service_time){
			 			$show_new_time= floor($i/60) . ":" . ($i/60-floor($i/60))*60; 
			 			 ...
			 			 $a=mysqli_query($con,"select * from worker where spe_id='$specilaist_id' and cat_id=$spe_cat_id and subcat_id=$spe_sub_cat_id"); 
			 			  // get worker details ontime only 
						//print_r(@mysqli_fetch_array($a));die;
			...
			// 			 		//print_r(array_unique($workerData));die;
			 				 	// if(array_diff($workerData,$workerArray) != 0){
			// 				 	// 	//echo '1';die;
			// 				 	//  	//array_push($workerData,array_unique($workerArray));
			// 		 	 	// 	}else{
			// 		 	 	// 		//echo '12';die;
			// 		 	 	// 		$workerData = array();
			// 		 	 	// 	}
			...
			//  				// already  occupied					
			 	 			array_push($afterNonTime,array('time'=>$show_new_time,'hasAppointment'=>true));
			 	 			$dateLoop = $current_date_loop;
			 			 	$dayLoop = $loop_day;				 			
			 	 	 	}
	...
	$datatext['message'] = "No Booking Parameters Fetched.";
	$logParameters = array(
			"Request_Remote_Address" => $remoteAddress,
			"Requested_Page" => 'get_booking_parameters',
			"Request_Method" => $requestType,
