        ...
        <div class="container">
          ...
          <div class="crumbs">
            <ul id="breadcrumbs" class="breadcrumb">
              <li class="current">
                <i class="fa fa-home"></i>User Growth
              </li>
            </ul>
          </div>
    ...
    //initial value of dataPoints 
     ...
     <?php 
         /* $sql_con = mysqli_query($con,"SELECT count(*) as month_count, DATE(created_at) as dat, MONTHNAME(created_at) as mon,YEAR(created_at)FROM user GROUP BY date(created_at),YEAR(created_at)");
             ...
             $sql_con = mysqli_query($con,"SELECT count(*) as coun,county FROM user GROUP BY county");
                               $count = 0;
                               while($row_con = mysqli_fetch_array($sql_con))
                               {  
                                    $month_count= (int) $row_con['coun'];
                                  $arrayName[] = array('label' =>$row_con['county'] , 'y' =>$month_count);
                               }
