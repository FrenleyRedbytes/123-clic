...
session_start();
  ...
  if(isset($_POST['submit'])){
	...
	if ($chk > 0){
		...
		$user_info = mysqli_query($con, "update register set `new_pass`='$pass_change'  where `email`='$email'");
		if ($user_info){
			$to = $email;
			$subject = 'Password Reset';
			$headers = "From: " . strip_tags('contact@123-clic.fr') . "\r\n";
			$headers.= "MIME-Version: 1.0\r\n";
			$headers.= "Content-Type: text/html; charset=UTF-8\r\n";
			$message.= '<h4>123 clic</h4>
							<p>Merci de cliquer sur le lien ci-dessous pour modifier votre mot de passe</p>
							<a href="http://etango.fr/directory_ui/confirmpass.php?token='.$token.'">Click here..</a>';
            				
			mail($to, $subject, $message, $headers);
      header("location:forgot.php?msg=0");
    }
    else{
      ...
      <div class="col-sm-12 text-center">
        <div class="row">
          <div class="col-md-12 categories-heading bt_heading_3">
           <h1 style="color: #850035;">Mot de passe oublié</span></h1>
          ...
          ?>        
            ...
            Un lien pour modifier votre mot de passe vous a été envoyé
            </div> 
          <?php
          }else if ($_GET['msg'] == "1"){ ?>
          ...
          <div class="col-md-6">
          <input type="email" class="form-control" name="email" placeholder="Enter Email" style="margin-left: 67%;width: 74%;">
