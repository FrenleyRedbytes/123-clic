...
include 'db.php';
  ...
  $service_time=$data_ar11['time'];
    ...
    <?php } ?>
    <span class="table_icon right"> <img src="images/right1.png" onclick="right('<?php echo date("Y-m-d", $date); ?>','<?php echo $spe_id; ?>','<?php echo $ser_id; ?>')">
     </span>
    </div>
    <div class="body_div">
    <?php for($k=2;$k>=0;$k--)
    ...
    // if($i%2==0){ 
      $date12=strtotime("-$k day", strtotime($_GET['date1']));
  ...
  $start_time=$str_pm;
   ...
   for($i=$start_time*60+$start_time1[1];$i<=$end_time*60+$end_time1[1];$i+=15){
    ...
    <?php }else{ ?>
    <div class="body_part">
    <button type="button" onclick="valid('<?php echo date('l',strtotime("-$k day")); ?>','<?php echo date('Y-m-d',$date12); ?>','<?php echo$showdata; ?>','<?php echo $spe_id; ?>','<?php echo $ser_id; ?>');"> <?php echo $show_new_time; ?> </button>
    </div>
    <?php } 
  }// for loop....
